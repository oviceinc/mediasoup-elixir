...
pub fn webrtc_transport_event(
    ...
    if event_filter.contains(&atoms::on_ice_state_change()) {
        crate::reg_callback_json_param!(pid, transport, on_ice_state_change);
    }
    if event_filter.contains(&atoms::on_dtls_state_change()) {
        crate::reg_callback_json_param!(pid, transport, on_dtls_state_change);
    }

    if event_filter.contains(&atoms::on_ice_selected_tuple_change()) {
        transport
