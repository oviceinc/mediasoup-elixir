...
defmodule Mediasoup.EventListenerTest do
  ...
  test "add/4 adds listener with tag" do
    listener = Mediasoup.EventListener.new()
    listener = Mediasoup.EventListener.add(listener, self(), [:on_close], :test_tag)
    assert Map.has_key?(listener.listeners, self())
    assert listener.listeners[self()].tag == :test_tag
  end
  ...
  test "send/3 sends message with tag to listeners" do
    listener = Mediasoup.EventListener.new()
    listener = Mediasoup.EventListener.add(listener, self(), [:on_close], :test_tag)
    Mediasoup.EventListener.send(listener, :on_close, {:on_close})
    assert_receive {:on_close, :test_tag}
  end
